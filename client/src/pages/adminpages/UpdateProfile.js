...
const UpdateProfile = () => {
  ...
  const [updateValue, setupdateValue] = useState(EditUserValue)
      ...
      <section>
        <div className="bg-white  rounded-lg shadow relative m-10">
          ...
          <div className="p-6 space-y-6">
            <form onSubmit={UserSubmit} enctype="multipart/form-data" method="post">
              <div className="grid grid-cols-10 gap-6">

                <div className="col-span-6 sm:col-span-3">
                  <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="avatar">Select An Image File (Max 0.5 MB)</label>
                  <input onChange={(e) => handleImageChange(e)} name='avatar' class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" aria-describedby="file_input_help" id="avatar" type="file" />
                  <p class="mt-1 text-sm text-gray-500 dark:text-gray-300" id="avatar">SVG, PNG, JPG or GIF (MAX. 800x400px).</p>
                </div>
              ...
              <div className="p-6 border-t border-gray-200 rounded-b">
                <button onClick={UserSubmit} type="submit" className="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Update User</button>
              </div>
            </form>
          </div>
          <ToastContainer />
